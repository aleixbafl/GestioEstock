...
Public Class Eliminar_Categories
    ...
    Private Sub DataGridView1_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles DataGridView1.CellMouseClick
        ...
        If e.Button = MouseButtons.Right Then
            Dim isbn As String = DataGridView1.Rows(e.RowIndex).Cells("ID_Categoria").Value.ToString()
            If MessageBox.Show("¿Estás seguro de que deseas eliminar este Registro?", "Eliminar Registro", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                ...
                Dim query As String = "DELETE FROM categories WHERE ID_Categoria = @ID_Categoria;"
                Using conn As New MySqlConnection(connString), cmd As New MySqlCommand(query, conn)
                    cmd.Parameters.AddWithValue("@ID_Categoria", isbn)
                    conn.Open()
                    cmd.ExecuteNonQuery()
                End Using
                DataGridView1.DataSource = GetData()
